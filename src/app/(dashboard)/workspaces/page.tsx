...
import { UsersIcon, DollarSignIcon, LaptopIcon, SearchIcon, CalendarCheckIcon } from "lucide-react";
  ...
  const handleBook = async () => {
    ...
    try {
      const res = await fetch("/api/reservations", {
        ...
        body: JSON.stringify({ workspaceId: selectedWorkspace.id, reservationDate: date.toISOString() }),
      });
      if (res.ok) {
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {filteredWorkspaces.map((ws) => (
            <Card key={ws.id} className="glass flex flex-col">
              ...
              <CardFooter>
                <Dialog open={dialogOpen && selectedWorkspace?.id === ws.id} onOpenChange={(open) => { setDialogOpen(open); if (!open) { setDate(undefined); setBookedDates(new Set()); setRangeMode(false); setDateRange({}); setRangeSummary(null); } }}>
